<?php
/**
 * Contains Wpvr_Fform_Dependency_Checker class. and wps_extend_plugins function.
 *
 * @package    WPS_Core
 ...
 */
if (!class_exists('Wpvr_Fform_Dependency_Checker'))
{
    /**
     * Class Wpvr_Fform_Dependency_Checker
     *
    ...
    class Wpvr_Fform_Dependency_Checker
    {
        ...
        public function __construct($plugin, $root_file, $min_version = null, $text_domain = null)
        {
            ...
            */
            if ('plugins.php' === basename($_SERVER['PHP_SELF']) && !(defined('WP_CLI') && WP_CLI))
            {
                ...
                add_action('after_plugin_row_' . plugin_basename($root_file) , array(
                    $this,
                    'plugin_row'
                ));
            }
            else
            {
                ...
                add_action('update_option_active_sitewide_plugins', array(
                    $this,
                    'maybe_deactivate'
                ) , 10, 2);
                add_action('update_option_active_plugins', array(
                    $this,
                    'maybe_deactivate'
                ) , 10, 2);
            }
        ...
        public function plugin_action_links_maybe_deactivate($actions)
        {
            if (!$this->is_active()) {
                if ( is_multisite() ) {
                    self::deactivate_self( $this->root_file, true );
                }else {
...
//                    $this->message = sprintf(__('You must have <a href="%s" target="_blank"><strong>WPVR PRO</strong></a> to install the Fluent Forms module.', $this->text_domain), 'https://rextheme.com/wpvr#pricing');
//                    $this->message = sprintf(
//                        __( '%1$s (v%2$s) is required for <strong>%3$s</strong>. Deactivating %3$s.', $this->text_domain ),
//                        $dependency,
//                        $this->min_version,
//                        $current
//                    );
                    $this->message = __( 'You need WPVR Pro (v3.8) activated to use this Add on. So please activate WPVR Pro and then try again.', $this->text_domain );
...
//                    );
        ...
        public static function deactivate_self($file, $network_wide = false)
        {
            if (is_multisite() && false !== $network_wide) {
                $network_wide = is_plugin_active_for_network( 'wpvr-fluent-forms-add-on/wpvr-fluent-form-addon.php' );
            }
